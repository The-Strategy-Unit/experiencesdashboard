...
mod_demographics_selection_server <- function(id, filter_data){
  moduleServer( id, function(input, output, session){
    ...
    output$dynamic_demographics_selection <- renderUI({
      ...
      tagList(
          if(has_age){uiOutput(ns("age_UI"))},
          if(has_gender){uiOutput(ns("gender_UI"))},
          if(has_ethnicity){uiOutput(ns("ethnicity_UI"))}
      )
    })
...
# mod_demographics_selection_server("demographics_selection_1")
